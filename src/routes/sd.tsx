...
export function SD() {
  ...
  const sdContentsQuery = useQuery(
    ["sd_contents", path],
    ({ queryKey }) => fetchSDContents(queryKey[1]),
    {
      staleTime: 600_000,
      retryDelay: 60_000,
  ...
  if (sdContentsQuery.isError)
    return <div>Error: {`${sdContentsQuery.error}`}</div>;
                ...
                >
                  <ContextMenu>
                    <ContextMenuTrigger asChild>
                      <div
                        onClick={() =>
                          type === "dir" ? setPath(name) : setFilePath(name)
                        }
                  ...
                  >
                    <Play
                      onClick={() =>
                        playFile({ file: name, mode: "once", power })
                      }
                      size={32}
                    ...
                    <RotateCw
                      onClick={() =>
                        playFile({ file: name, mode: "repeat", power })
                      }
                      size={32}
                      className="h-[100%] p-1 transition-all hover:cursor-pointer hover:bg-stone-600"
                    />
                    <ArrowRightToLine
                      onClick={() =>
                        playFile({ file: name, mode: "beginPosOnly", power })
                      }
                      size={32}
                      className="h-[100%] p-1 transition-all hover:cursor-pointer hover:bg-stone-600"
                    />
